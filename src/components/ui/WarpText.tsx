...
const drawLine = (ctx: CanvasRenderingContext2D, line: string, x: number, y: number, letterSpacing: number) => {
  ...
  if (parts.length === 2 && parts[0] === "TEAM") {
    const totalWidth = measureLine(ctx, line, letterSpacing);
    let cursor = x - totalWidth / 2;

    // Draw "TEAM " in crisp high-contrast white
    const teamText = "TEAM ";
    const teamChars = Array.from(teamText);
    ...
    // Draw Season (e.g. "2025-26") in Mach-Red Kinetic gradient (#DE1615 -> #FF6534)
    const seasonText = parts[1];
    const seasonWidth = measureLine(ctx, seasonText, letterSpacing);
    const seasonGrad = ctx.createLinearGradient(cursor, y, cursor + seasonWidth, y);
    seasonGrad.addColorStop(0, "#DE1615");
    seasonGrad.addColorStop(1, "#FF6534");
    ctx.fillStyle = seasonGrad;

    const seasonChars = Array.from(seasonText);
    seasonChars.forEach((char, index) => {
      ctx.fillText(char, cursor, y);
      cursor += ctx.measureText(char).width + (index === seasonChars.length - 1 ? 0 : letterSpacing);
    });
    return;
  }
