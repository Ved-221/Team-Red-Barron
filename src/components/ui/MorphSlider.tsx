...
void main() {
  ...
  bool inC = uContain < 0.5 || (sC.x >= 0.0 && sC.x <= 1.0 && sC.y >= 0.0 && sC.y <= 1.0);
  bool inN = uContain < 0.5 || (sN.x >= 0.0 && sN.x <= 1.0 && sN.y >= 0.0 && sN.y <= 1.0);

  vec3 colC = inC ? vec3(
    texture2D(tCurrent, clamp(sC + vec2(ca, 0.0), 0.0, 1.0)).r,
    texture2D(tCurrent, clamp(sC, 0.0, 1.0)).g,
    texture2D(tCurrent, clamp(sC - vec2(ca, 0.0), 0.0, 1.0)).b
  ) : vec3(0.05, 0.05, 0.06);

  vec3 colN = inN ? vec3(
    texture2D(tNext, clamp(sN + vec2(ca, 0.0), 0.0, 1.0)).r,
    texture2D(tNext, clamp(sN, 0.0, 1.0)).g,
    texture2D(tNext, clamp(sN - vec2(ca, 0.0), 0.0, 1.0)).b
  ) : vec3(0.05, 0.05, 0.06);
...
class MorphEngine {
  ...
  constructor(
    ...
    const isContainInit = opts.fitMode === 'contain' || (opts.fitMode !== 'cover' && isMobileInit);
    ...
    this.program = new Program(this.gl, {
      ...
      uniforms: {
        ...
        uContain: { value: isContainInit ? 1 : 0 },
  ...
  syncOptions() {
    ...
    const isContain = opts.fitMode === 'contain' || (opts.fitMode !== 'cover' && isMobile);
    this.program.uniforms.uContain.value = isContain ? 1 : 0;
...
export default function MorphSlider({
  ...
  optsRef.current = { transition, duration, ease, intensity, scale, aberration, drift, overlayColor, loop, fitMode };
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver(
      ([entry]) => {
        ...
        if (!isVisible && engine.raf) {
          cancelAnimationFrame(engine.raf);
          engine.raf = 0;
        } else if (isVisible && !engine.raf) {
          engine.raf = requestAnimationFrame(engine.boundLoop);
        }
      },
      { threshold: 0 }
    );
